...
with open('calls.csv', 'r') as f:
    ...
    for record in calls:
        if record[0] in result:
            result[record[0]] += int(record[3])
        else:
            result[record[0]] = int(record[3])

        if record[1] in result:
            result[record[1]] += int(record[3])
        else:
            result[record[1]] = int(record[3])

    result_sorted = sorted(result.items(), key=lambda kv: kv[1])
    print(
        f"{result_sorted[-1][0]} spent the longest time, {result_sorted[-1][1]} seconds, on the phone during September 2016.")
